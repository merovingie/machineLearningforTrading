...
def mySMA(prices, window):
    ...
    sma_50 = simpleMovingAverage(prices, 50)
    return simpleMovingAverage(prices, window), sma_price, sma_50
...
def graphIndicators(symbol = "JPM", sd = dt.datetime(2008, 1, 1), ed = dt.datetime(2009, 12, 31)):
    ...
    # SMA vs Long-term SMA
    fig, ax = plt.subplots(figsize=(14, 6))
    ax.set(xlabel='Time', ylabel="Price", title="Cross Over Simple Moving Average")
    ax.plot(prices, "red", label="Normalized Price")
    ax.plot(sma, "purple", label=f"SMA {window} days")
    plt.plot(sma_50_days, 'blue', label='SMA 50 days')
    ax.legend(loc="best")
    fig.savefig('images/rolling_window_SMA.png')
    plt.close()
